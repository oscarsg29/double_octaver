...
juce::AudioProcessorValueTreeState::ParameterLayout Curso032026AudioProcessor::createParameters()
{
    juce::AudioProcessorValueTreeState::ParameterLayout parameters;
    
    parameters.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID ("Gain", 1),
                                                                 "Gain",
                                                                 Gain::MinGainDb,
                                                                 Gain::MaxGainDb,
                                                                 Gain::UnityGainDb));

    /*parameters.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("Panning", 1),
                                                                 "Panning",
                                                                 -100.0f,
                                                                 100.0f,
                                                                 0.0f));

    parameters.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("LFO", 1),
                                                               "LFO",
                                                               0.01f,
                                                               20.0f,
                                                               20.0f));*/
   ...
   /* parameters.add(std::make_unique<juce::AudioParameterInt>(juce::ParameterID("Int", 1),
    ...
    parameters.add (std::make_unique<juce::AudioParameterBool> (juce::ParameterID ("Bool", 1),
                                                                "Bool",
                                                                true));
    
    parameters.add (std::make_unique<juce::AudioParameterChoice> (juce::ParameterID ("Choice", 1),
                                                                  "Choice",
...
double Curso032026AudioProcessor::getTailLengthSeconds() const
{
    return 0.0;
}
...
int Curso032026AudioProcessor::getCurrentProgram()
{
    return 0;
}
...
void Curso032026AudioProcessor::changeProgramName (int index, const juce::String& newName)
{
}
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool Curso032026AudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void Curso032026AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
...
//==============================================================================
void Curso032026AudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void Curso032026AudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
